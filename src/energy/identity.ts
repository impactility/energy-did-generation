...
export const defaultIdentityCreationOptions: IdentityCreationOptions = {
  ...
  // networkId: 'volta',
  method: core.DidMethod.Energy,
  blockchain: core.Blockchain.EnergyWeb,
  networkId: core.NetworkId.Volta,
  revocationOpts: {
    type: CredentialStatusType.Iden3ReverseSparseMerkleTreeProof,
    id: rhsUrl,
  },
  // seed: new TextEncoder().encode('seedseedseedseedseedseedseedseed'),
};
...
export async function identityCreation(identitySeed: string) {
  console.log('=============== key creation ===============');

  const { identityWallet } = await initInMemoryDataStorageAndWallets(
    defaultNetworkConnection,
  );
  const identityCreationOptionsWithSeed = {
    ...defaultIdentityCreationOptions,
    seed: new TextEncoder().encode(identitySeed),
  };
  const { did, credential } = await identityWallet.createIdentity(
    identityCreationOptionsWithSeed,
  );
...
async function generateProofs(useMongoStore = false) {
  ...
  if (useMongoStore) {
    ({ dataStorage, credentialWallet, identityWallet } =
      await initMongoDataStorageAndWallets(defaultNetworkConnection));
  } else {
    ({ dataStorage, credentialWallet, identityWallet } =
      await initInMemoryDataStorageAndWallets(defaultNetworkConnection));
  }
